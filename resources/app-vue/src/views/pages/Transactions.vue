...
const saveTransaction = async () => {
    ...
    if (transaction.value.name && transaction.value.name.trim() && transaction.value.amount) {
        if (transaction.value.id) {
            ...
            const index = walletTransactions.value.findIndex((t) => t.id === transaction.value.id);
            if (index !== -1) {
                walletTransactions.value[index] = editedTransaction;
            }
        ...
        <div class="col-12">
            <div class="card">
                ...
                >
                    ...
                    <Column field="amount" header="Amount" :sortable="true" headerStyle="width:20%; min-width:8rem;">
                        <template #body="slotProps">
                            <span class="p-column-title">Amount</span>
                            {{ formatCurrency(slotProps.data.amount) }}
                        </template>
                    </Column>
                    <Column field="type" header="Type" :sortable="true" headerStyle="width:20%; min-width:10rem;">
                        <template #body="slotProps">
                        ...
                        <Dropdown optionValue="value" id="inventoryStatus" v-model="transaction.status" :options="statuses" optionLabel="label" placeholder="Select a Status">
                            <template #value="slotProps">
                                ...
                                <div v-else-if="slotProps.value && !slotProps.value.value">
                                    <span :class="'transaction-badge status-' + slotProps.value.toLowerCase()">{{ slotProps.value }}</span>
                                </div>
                                <span v-else>
